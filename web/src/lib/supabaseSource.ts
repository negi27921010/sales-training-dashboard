...
export async function loadAll() {
  if (!supabase) throw new Error('Supabase env vars missing — set VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY');
  ...
  const sessionsById = new Map<number, SessionDef>();
  const sessions: SessionDef[] = (sessRes.data ?? []).map(r => {
    const s: SessionDef = {
  ...
  const batchSessions: BatchSession[] = (bsRes.data ?? []).map(b => ({
  ...
  const attendance: AttendanceRow[] = (attRes.data ?? []).map(a => ({
  ...
  const assessments: AssessmentRow[] = (asmRes.data ?? []).map(a => ({
    email: empById.get(a.employee_id) ?? '',
    sessionNumber: sessionsById.get(a.session_id)!.sessionNumber,
