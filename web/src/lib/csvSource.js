...
import { COMPETENCIES, ZONE_BY_BATCH, } from '../types';
const SEED = '/seed';
...
export async function loadAll() {
    ...
    for (const r of dataRows) {
        ...
        if (!code || /^column|^topics?$/i.test(code))
            continue;
        const number = parseInt(code.match(/(\d+)/)?.[1] ?? '0', 10);
        if (!number)
            continue;
        const topic = (r[1] || '').toString().trim();
        const sessionType = /^basics/i.test(code) ? 'Basics' : number === 14 ? 'Ceremony' : 'Deep Dive';
        ...
        for (let b = 1; b <= 5; b++) {
            ...
            if (!date && !trainer)
                continue;
            const status = date && new Date(date) < new Date() ? 'completed' : 'scheduled';
            batchSessions.push({
    ...
    for (const r of attRows.slice(1)) {
        ...
        if (!email)
            continue;
        const role = (['ZM', 'BDM', 'BDA', 'Exit'].includes((r[3] || '').trim()) ? r[3].trim() : 'BDA');
        const emp = {
            email,
        ...
        for (let s = 1; s <= 10; s++) {
            ...
            const status = cell === 'YES' ? 'present'
                : cell === 'NO' ? 'absent'
                    : cell === 'R' ? 'rescheduled'
                        : null;
            attendance.push({ email, sessionNumber: s, batch, status: status });
        }
    }
