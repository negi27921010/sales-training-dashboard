...
-- Allow anon read for dashboard (Phase 1). Writes happen only via service-role from Apps Script.
do $$ begin
  for t in select unnest(array[
    ...
    execute format(
      'drop policy if exists "read_all_%1$s" on %1$I;
       create policy "read_all_%1$s" on %1$I for select using (true);', t);
  end loop;
end $$;
